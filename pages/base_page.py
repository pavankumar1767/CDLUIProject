...
def _attach_log_to_allure(log_message, label=None):
    """
    Attaches a log message to the Allure report with a custom label.

    :param log_message: The log message to attach.
    :param label: Optional label to use as the attachment name in Allure.
    """
    name = label if label else log_message.split(":")[0]
    allure.attach(log_message, name=name.strip(), attachment_type=allure.attachment_type.TEXT)
...
class BasePage:
    ...
    def get_text_list(self, selector, element_name="Element"):
        ...
        if elements.count() == 1:
            ...
            cleaned_text = text.strip() if text else ""
            self._capture_screenshot(f"{cleaned_text} Text Retrieved from {element_name}")
            _attach_log_to_allure(f"Text retrieved from {element_name}: '{cleaned_text}'", label="Text Content")
            return cleaned_text
        else:
            texts = elements.all_text_contents()
            stripped_texts = [text.strip() for text in texts if text]
            if stripped_texts:
                self._capture_screenshot(f"{element_name} - Text Retrieved: {stripped_texts[0]}")
                _attach_log_to_allure(f"Texts retrieved from {element_name}: {stripped_texts}", label="Text List")
            else:
                _attach_log_to_allure(f"No text found for {element_name}", label="Text Missing")
            return stripped_texts
