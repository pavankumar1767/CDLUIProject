...
class TestLogin:
    ...
    @allure.title("Login_07 : Confirm that logging out redirects the user to the login page.")
    def test_logout(self, setup, config):
        page = setup
        login_page = LoginPage(page)
        ...
        assert login_page.get_title(config.username) == config.username
        login_page.logout()
        user_page.AssertButtonVisibility("Sign In")
